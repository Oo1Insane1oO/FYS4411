...
{#2}}
    ...
    given in the local energy and arrive at
        \begin{equation}
            E_L = \frac{1}{2}\suml{k}{N}\left(\omega^2r^2_k -
            \nabla^2_k\phi_j(r_k)\psi^{-1}_{jk}(\vec{r})\right)
            \sum\limits_{i<j} \frac{1}{r_{ij}}
            \label{eq:ELgenfir}
        \end{equation}
    See \Autoref{sub:optimization} for a more clear derivation for the above
    expression. We now need the derivative to the single particle wave
    functions. Start by defining a new function $g(\vec{r},\beta)$ as the
    Jastrow-factor
        \begin{equation}
            g(\vec{r},\beta) \equiv \prod_{i<j} \gJ
            \label{eq:g}
        \end{equation}
    and apply \Autoref{eq:nablaN} to the single particle wave function
    BLAAARGGG
        \begin{align}
            \nabla^2_N\psi^2_T &=
            ...
            \label{eq:lapPsiT1}
        \end{align}
    We have used a short-hand notation $\det(\Phi)$ defined as
        \begin{equation}
    ...
    expansion given in \Autoref{sub:laplace_expansion}. The Laplace expansion
    simplifies the expression due to the fact that the cofactors are
    independant of the $k$-th particle. This gives us
        \begin{align}
            \nabla^2_k\det(\Phi(\vec{r},\alpha)) &=
            \nabla^2_k\suml{i}{N}C_{ij}\Phi_{ij} \nonumber \\
            &= \nabla_k\cdot\left(C_{kj}\nabla_k\Phi_{kj}\right) \nonumber \\
            &= C_{kj}\nabla^2_k\Phi_{kj}
            \label{eq:nablakPhilapexp}
        \end{align}
    The single particle
    wave function is
        \begin{equation}
    ...
    The expression to be solved for the $k$-th particle is thus
        \begin{equation}
            \begin{aligned}
                ...
                \prd{x_k}[][2]\left(H_{n_{x_j}}\left(\sqrt{\omega}x_k\right)
                \exp(-\frac{\alpha\omega}{2}x^2_k)\right) \\
                &+ H_{n_{x_j}}\left(\sqrt{\omega}x_k\right)
                \exp(-\frac{\alpha\omega}{2}x^2_k)
                \prd{y_k}[][2]\left(H_{n_{y_j}}\left(\sqrt{\omega}y_k\right)
    ...
    in \Autoref{eq:hermiteReq} for $H_n(s)$ gives
        \begin{align}
            ...
            \omega\left(\prd{x}[H_n][2] - 2\alpha s\prd{x}[H_n] +
...
\newcommand{\rold}{r^{\text{old}}}
    ...
    And a similarly for the Laplacian
        \begin{equation}
            ...
            \suml{j}{N}\nabla^2_i\phi_j(\rnew_i)\phi^{-1}_{ji}(\rold)
            \label{eq:Rdersec}
        \end{equation}
