...
\newcommand{\prodsecrule}[2]{{#2} \nabla^2_k {#1} + {#1} \nabla^2_k {#2}}
    ...
    in \Autoref{eq:hermiteReq} for $H_n(s)$ gives
        \begin{align}
            ...
            \nonumber \\
            &= \omega \left[4(n-1)\left(n(2+\alpha) - \alpha\left(\alpha s^2
            - 1\right)\right)\frac{H_{n-2}}{H_n} + \alpha\left(\alpha s^2 - 1
            - 2n\right)\right]H_n(s)e(s)
            \label{eq:diffs2}
        \end{align}
    ...
    Defining
        \begin{equation}
            \begin{aligned}
                ...
                4\omega(n_{x_k}-1)\left(n_{x_k}(2+\alpha) - \alpha\left(\alpha
                \omega x_k^2 - 1\right)\right)\frac{H_{n_{x_k}-2}}{H_{n_{x_k}}}
                \\
                &+ 4\omega(n_{y_k}-1)\left(n_{y_k}(2+\alpha) -
                \alpha\left(\alpha \omega y_k^2 -
                1\right)\right)\frac{H_{n_{y_k}-2}}{H_{n_{y_k}}} \\
