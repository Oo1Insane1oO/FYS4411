...
\begin{center} \vspace{1cm}
    ...
    \textbf{\large{\massignn\hspace{0.01pt} - Variational Monte Carlo Studies of
...
\begin{center}
    \textbf{Abstract} \\
    We have used a variational monte carlo  method for calculating the
    ground-state energies for a quantum-dot system. The results are promising
    and are lower then the energies obtained from the Hartree-Fock method.
\end{center}
...
\label{sec:introduction}
    ...
    state energy with the variational principle and minimize the energy
    samples. The algorithm used for the Monte Carlo method is the well known
    Metropolis algorithm(with a Hastings approach).
...
\label{ssub:Blocking}
    The method of blocking estimates the standard deviation. We start by first
    defining a sample quantity $\chi$ of $n$ samples and assume that the
    standard deviation of the mean of $\chi$ is given by
        \begin{equation}
            \sigma_{\chi} = \sqrt{\frac{1}{n}\left(\ecp{\chi^2} -
            \ecp{\chi}^2\right)}
            \label{eq:sigmachidef}
        \end{equation}
    Obviously this is wrong if $\chi$ is a set of correlated samples. However,
    we can rewrite \Autoref{eq:sigmachidef} with \Autoref{eq:autocorrtime}
    giving
        \begin{equation}
            \sigma_{\chi} = \sqrt{\frac{1+\frac{2\tau}{\Delta
            t}}{n}\left(\ecp{\chi^2} - \ecp{\chi}^2\right)}
            \label{eq:sigmachicorrdef}
        \end{equation}
    where $\Delta t$ is the time between each sample. \\
    If we have that $\Delta t \gg\tau$ then the assumption for non-correlation
    still hold, if this is not the case we may use the method of blocking. The
    method is to basically divide the sample sequence into blocks of size
    $\tau$(meaning sample $j$ of block $i$ is uncorrelated to sample $j$ of
    block $i+1$). The total mean and variance is then obtained by calculating
    the mean of blocks $\chi_i,\dots,\chi_{n_{\text{blocks}}}$. \\
    The second problem is to find $\tau$. This is solved by making a plot of
    the standard deviation and divide the blocks accordingly. The standard
    deviation will increase and reach a convergence as the block size
    increases.

\section{SETUP}
\label{sec:setup}
    See \url{https://www.github.com/Oo1Insane1oO/FYS4411} for C++ program for
    the VMC calculations. \\
    The program sets up the basis in Cartesian coordinates(class Basis in
    basis.cpp) and sets up the wave function before running the calculations. \\
    All stand alone methods are given in class Methods(methods.cpp) and the VMC
    part is done in class VMC. \\
    The main file sets up the class objects and takes care of dividing the
    problem for MPI(and runs with MPI). The variational parameters are gathered
    to root process in the end and averaged. All the processes write to their
    own respective files. \\
    The repository also contains a python script file for the blocking
    analysis(readBinary.py). \\
    The function which runs the VMC calculations splits the wave function in
    two, one for spin up states and one for spin down and uses the different
    optimizations mentioned in \Autoref{sub:optimization}(functions themselves
    are given in methods.cpp). After the Metropolis sampling is done, we use
    the Steepest descent method for minimization. \\
    The whole sampling is run one last time with the optimal parameters and
    results are dumped to binary files(one for each process). \\
    There is also a class Tests(tests.cpp) which just a collection of different
    tests for the different classes mentioned.
...
\label{sec:results}
    \begin{table}[H]
        ...
        \begin{tabular}{cccccccc} \hline\hline
            $N$ & $\omega$ & $\ecp{E}$ & $\sigma^2$ & $\ecp{V}$ & $\ecp{K}$ &
            $\alpha$ & $\beta$ \\ \hline
            \input{dataTexTable.txt} \hline
        \end{tabular}
    \end{table}
    
    \begin{table}[H]
        \centering
        \textbf{Table of Results(Without Jastrow)} \\
        \vspace{0.3cm} 
        \begin{tabular}{cccccccc} \hline\hline
            $N$ & $\omega$ & $\ecp{E}$ & $\sigma^2$ & $\ecp{V}$ & $\ecp{K}$ &
            $\alpha$ & $\beta$ \\ \hline
            \input{dataWOJTexTable.txt} \hline
        \end{tabular}
    \end{table}

\twofigure{text/figures/Blocksize_w010_N2_WOJ.pdf}{Blocking for $N=2$ and
$\omega=1$}{blockN2w1}{text/figures/Blocksize_w01_N6_WOJ.pdf}{Blocking for $N=6$
and $\omega=1$}{blockN6w1}{Plots for $\omega=1$ and $M=1\times 10^{8}$ Monte
Carly cycles.}
\twofigure{text/figures/Blocksize_w010_N12_WOJ.pdf}{Blocking for $N=12$ and
$\omega=1$}{blockN12w1}{text/figures/Blocksize_w010_N20_WOJ.pdf}{Blocking for $N=20$
and $\omega=1$}{blockN20w1}{Plots for $\omega=1$ and $M=1\times 10^{8}$ Monte
Carly cycles.}

The rest of the figures are given in the mentioned github repository.\\

    \begin{table}[H]
        \centering
        \textbf{Table of Runtime} \\
        \vspace{0.3cm} 
        \begin{tabular}{ccc|cc} \hline\hline
            & \multicolumn{2}{c}{Parallelized} & \multicolumn{2}{c}{Serial} \\
            Num Proc & Vectorized & Not Vectorization & Vectorized & Not Vectorized \\
            \input{runtimeVec.txt} \\ \hline
        \end{tabular}
    \end{table}
    The times are given with an average over 1000 runs and 1000000 Monte Carlo
    cycles.

\section{DISCUSSION}
\label{sec:discussion}
\subsection{With Jastrow}
\label{sub:with_jastrow}
\subsubsection{Energies}
\label{ssub:Energies}
    As we can see from \Autoref{sec:results} the energies are lower then the
    energies found from Hartree-Fock method, especially for lower values of
    oscillator frequency($\omega$). This is dues to fact that the VMC method
    does not have a limit which the Hartree-Fock methods has.

\subsubsection{Error}
\label{ssub:Error}
    The standard deviation is well within the assumed range and stays roughly
    the same as we increase the number of particles. This increase is natural
    as we have more variance with more electrons in the system, however the
    increase is only marginal and acceptable. \\
    For the blocking part we have plots which have the expected form that are
    convergent. The blocking analysis was performed for only a selected number
    of block sizes until a convergence was observed. The analysis could have
    been further improved by introduced all the possible block sizes(just keep
    splitting the data set in two).

\subsection{Without Jastrow}
\label{sub:without_jastrow}
    For the energies without the Jastrow factor we can see that it does not
    reach the minimum. This is dues to the cusp condition introduced by the
    Jastrow factor. Removing that particular condition also removes the desired
    cusp form or the energy, meaning the minimization does not reach the actual
    minimum as one can see.

\subsection{Speed-up}
\label{sub:speed_up}
    We see that with MPI the speed-up almost follows by a factor number of
    processes, suggesting we have an almost perfect speed-up ratio per number
    of processes. This is natural since we basically split the iteration of the
    program into individual pieces which as independent of each process(that is
    to say there is no communication between the processes).\\
    As for the vectorization we see that we have more then a doubling in speed.
    One note on the vectorization is that we have only made a simple
    optimization on this manner by leaving the whole vectorization scheme to
    the compiler entirely. Most of the loops in the program can still be
    improved.

\section{CONCLUSION}
\label{sec:conclusion}
    We have in this project set up a program for calculating the ground-state
    energy of an electron system trapped in a quantum-dot with the harmonic
    oscillator functions as basis functions. \\
    The resulting energies look good and are lower then the calculations made
    with the Hartree-Fock method as expected. \\
    The program can still be optimized in terms of vectorization and the script
    for the blocking analysis can be much improved by parallelization. \\
    The Blocking analysis would give a clearer view of the method of Blocking
    is all block sizes was introduced, how ever the optimal standard deviation
    was obtained in our calculation. \\
    The goal of the calculations made were to see if the VMC method is good at
    estimating the energy of our quantum-dot system, to which we can conclude
    it is, or at least better then the Hartree-Fock method.
    ...
    and the Laplacian
        \begin{equation}
            \begin{aligned}
                ...
                \phi_j\left(\vec{r}_k,\alpha\right)
                \phi^{-1}_{jk}\left(\vec{r},\alpha\right) \\
                &= \alpha\omega\suml{k,j}{N} \left(\alpha\omega r^2_k -
    ...
    \Autoref{eq:jastoptfirstder})
        \begin{equation}
            \begin{aligned}
                ...
                r_{ki}\right)^2} \\
                &= \sum\limits_{i\neq k} \frac{a\left(\vec{r_k} -
                \vec{r_i}\right)}{r_{ki}\left(1+\beta r_{ki}\right)^2}
            \end{aligned}
...
\subsection{ANALYTIC EXPRESSION FOR DERIVATIVES OF VARIATIONAL PARAMETERS}
\label{ssub:analytic_expression_for_derivatives_of_variational_parameters}
...
\newcommand{\gsum}{\sum\limits_{i\neq j} \frac{a}{\left(\beta + \frac{1}{r_{ij}}\right)}}
    The first derivatives of the wave function with respect to the variational
    parameters is needed in order to minimize with gradient descent method as
    described in \Autoref{sub:gradient_descent}.  We solve the derivatives
    generally for a variational parameters $c_n$. The first derivative is thus
        \begin{align}
            \prd{c_n}[\ecp{E_L}] &=
...
\newcommand{\eri}{\exp(-\frac{\alpha\omega}{2}r^2_i)}
    ...
    individual Hermite polynomials. These are simply (with the  chain rule)
        \begin{align}
            ...
            \\
            &= \frac{n_{x_j}}{2\alpha}\left(1 +
            x_i\sqrt{\frac{\omega}{\alpha}}\left(n_{x_j}-1\right)
    ...
    with
        \begin{equation}
            \Theta_{ij}(\alpha,x) \equiv \frac{n_{x_j}}{\alpha}\left(1 +
            x_i\sqrt{\frac{\omega}{\alpha}}\left(n_{x_j}-1\right)
    ...
    $\psi_T$ is
        \begin{align}
            ...
            \nonumber \\
            &= \frac{\psi_T}{2} \suml{i,j}{N} \Phi^{-1}_{ij}
            \Phi_{ji}\left(\left(\Theta_{ji}(\alpha,x) +
            \Theta_{ji}(\alpha,y)\right) - \omega r^2_j\right)
            \label{eq:psideralpha}
        \end{align}
    where we have used \Autoref{eq:aadja,eq:Thetadef}. \\
