...
\newcommand{\prodsecrule}[2]{{#2} \nabla^2_k {#1} + {#1} \nabla^2_k {#2}}
    ...
    The expression to be solved for the $k$-th particle is thus
        \begin{equation}
            ...
            \label{eq:nablakth}
        \end{equation}
    Solving the differential for $x_k$ by substituting $s=\sqrt{\omega}x_k$ and
    labeling $n_x\rarr n$ and defining
        \begin{equation}
            e_k(x_k) \equiv \exp(-\frac{\alpha\omega}{2}x^2_k)
            \label{eq:e}
        \end{equation}
    we have
        \begin{align}
            \prd{x_k}[][2]\left(H_{n_{x_k}}\left(\sqrt{\omega}x_k\right)
            \exp(-\frac{\alpha\omega}{2}x^2_k)\right) &= \sqrt{\omega}
            \prd{s}[][2]\left(H_n(s)e(s)\right) \nonumber
            \\
            &= \sqrt{\omega}e(s) \left(\prd{s}[H_n][2] -
            2\alpha s\prd{s}[H_n(s)] + \alpha(s-1)H_n(s)\right)
            \label{eq:diffxk1}
        \end{align}
    Using \Autoref{eq:hermiteDer} for the derivatives and using the recursion
    relation given in \Autoref{eq:hermiteReq} for $H_n(s)$ gives
        \begin{align}
            \sqrt{\omega} \prd{s}[][2]\left(H_n(s)e(s))\right) &=
            \sqrt{\omega}\left(8n(n-1)H_{n-2}(s) - 4n\alpha H_{n-1}(s) +
            \alpha(s-1)(2sH_{n-1}(s)-2(n-1)H_{n-2})\right)e(s) \nonumber \\
            &= \sqrt{\omega}\left[\frac{n}{s}(4(n+1)-\alpha(s-1))\me^{s^2} +
            2\alpha(s(s-1) - 2(n+1))\right]H_n(s)e(s) \\
            &= \left[4\left(n_{x_k} + 1\right)
            \left(\frac{n_{x_k}}{x_k}\me^{\omega x^2_k} -
            \sqrt{\omega}\alpha\right) + \alpha\left(\sqrt{\omega}x_k -
            1\right)\left(2\omega x_k - \frac{n_{x_k}}{x_k}\me^{\omega
            x^2_k}\right)\right]H\left(\sqrt{\omega}x_k\right)e\left(x_k\right)
            \label{eq:diffs2}
        \end{align}
    A similar approach for $y_k$ gives
       \begin{equation}
            \prd{y_k}[][2]\left(H_{n_{y_k}}\left(\sqrt{\omega}y_k\right)
            e\left(y_k\right)\right) = \left[4\left(n_{y_k} +
            1\right) \left(\frac{n_{y_k}}{y_k}\me^{\omega y^2_k} -
            \sqrt{\omega}\alpha\right) + \alpha\left(\sqrt{\omega}y_k -
            1\right)\left(2\omega y_k - \frac{n_{y_k}}{y_k}\me^{\omega
            y^2_k}\right)\right]H\left(\sqrt{\omega}y_k\right)e\left(y_k\right)
            \label{eq:diffyk}
       \end{equation}
    Inserting \Autoref{eq:diffs2,eq:diffyk} into \Autoref{eq:nablakth} with
        \begin{align}
            D_{n_{x_k}n_{y_k}}(x_k,y_k,\alpha) \equiv 
            \left[4\left(n_{x_k} + 1\right)
            \left(\frac{n_{x_k}}{x_k}\me^{\omega x^2_k} -
            \sqrt{\omega}\alpha\right) + \alpha\left(\sqrt{\omega}x_k -
            1\right)\left(2\omega x_k - \frac{n_{x_k}}{x_k}\me^{\omega
    ...
    The Laplacian for $g$ is derived as follows
        \begin{align}
            ...
            \label{eq:lapg}
        \end{align}
%     We define a function $g_{kj}$ as follows
%         \begin{equation}
%             g_{kj}\left(r_k,r_j,\beta\right) = \exp(\frac{a}{\beta +
%             \frac{1}{r_{kj}}})
%             \label{eq:gjk}
%         \end{equation}
    and solve for the $k$-th part of the sum
        \begin{align}
                ...
                r_{kj})}{r_{kj}(1+\beta r_{kj})^3} + \frac{a^2}{(1+\beta
                r_{kj})^4} \nonumber \\
                &= \frac{a}{r_{kj}(1+\beta r_{kj})^2}\left(\frac{1}{r_{kj}} -
