...
\begin{center}
    \textbf{Abstract} \\
    We have used the VMC method for calculating the ground-state energies for a
    quantum-dot system. The results do not look correct and we conclude with an
    error in the code.
\end{center}
...
\twofigure{text/figures/Blocksize_w01_N2.pdf}{Blocking for $N=2$ and
$\omega=1$}{blockN2w1}{text/figures/Blocksize_w01_N6.pdf}{Blocking for $N=6$
and $\omega=1$}{blockN6w1}{Plots for $\omega=1$}
\twofigure{text/figures/Blocksize_w01_N12.pdf}{Blocking for $N=12$ and
$\omega=1$}{blockN12w1}{text/figures/Blocksize_w01_N20.pdf}{Blocking for $N=20$
and $\omega=1$}{blockN20w1}{Plots for $\omega=1$}
...
\label{ssub:Energies}
    As we can see from \Autoref{sec:results} the energies are rather strange
    compared to the Hartree-Fock energies. Some of the energies obtained by the
    VMC calculations are higher, which they should not be. There may be several
    reasons for this, we might not have found the optimal parameters, or(much
    more likely) there is an error in the code.

\subsubsection{Error}
\label{ssub:Error}
    With that said, the standard deviation is fairly good, however it increases
    with the number of particles suggesting there might be an error in how the
    derivatives are calculated in the VMC loop. \\
    For the blocking part we have plots which have the expected form, but are a
    bit jagged. The optimal standard deviation was found by smoothing out the
    graph and taking an average over the most stationary area(the area with
    lowest gradient).

\subsection{Without Jastrow}
\label{sub:without_jastrow}
    As for the calculations without the Jastrow factor the energies are poor as
    well.
...
\label{sec:conclusion}
    ...
    The resulting energies do not look promising as they are higher than the
    ones obtained from Hartree-Fock calculations. \\
    We conclude with that the poor results are due to an error in the code.
