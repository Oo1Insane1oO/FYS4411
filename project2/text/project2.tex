...
\newcommand{\eri}{\exp(-\frac{\alpha\omega}{2}r^2_i)}
    ...
    individual Hermite polynomials. These are simply (with the  chain rule)
        \begin{align}
            ...
            \nonumber \\
            &= n_{x_j}x_k\sqrt{\frac{\omega}{\alpha}}\Hij{x}{n}{-1} \nonumber \\
            &= \frac{n_{x_j}\omega^2}{2}\left(1 +
            \left(n_{x_j}-1\right)x_i\sqrt{\frac{\omega}{\alpha}}
    ...
    with respect to $\alpha$ is thus
        \begin{align}
            ...
            r^2_i}{2}\right)\eri \nonumber \\
            &=\frac{\omega}{2}\left(\omega\Theta_{ij}(\alpha,x) +
            \omega\Theta_{ij}(\alpha,y) - r^2_i\right)\Phi_{ij}
            \label{eq:phideralpha1}
        \end{align}
    with
        \begin{equation}
            \Theta_{ij}(\alpha,x) \equiv n_{x_j}\left(1 +
            x_i\left(n_{x_j}-1\right) \sqrt{\frac{\omega}{\alpha}}
    ...
    $\psi_T$ is
        \begin{align}
            ...
            \det(\Phi)\frac{\omega}{2}\Tr(\Phi^{-1}\Phi\cdot \Theta^T)
            \nonumber \\
            &= \psi_T \frac{\omega}{2}\suml{i}{N}
            \left(\omega\left(\Theta_{ii}(\alpha,x) +
            \Theta_{ii}(\alpha,y)\right) - r^2_i\right)
            \label{eq:psideralpha}
        \end{align}
    where we have use \Autoref{eq:aadja} and defined $\Theta$ to be the vector
