...
\label{ssub:quantum_force}
    ...
    dependence at this point is zero for all $i$. With other words, the left
    hand side of \Autoref{eq:FokkerPlancTotal}. We also know that the drift
...
\label{ssub:Solution}
    Using Eulers method on the Langevin equation(\Autoref{eq:Langevin}) one
...
\label{ssub:Two Electron Case}
    ...
    using the product rule for differentiation we get
        \begin{equation}
            ...
            \exp(\frac{ar_{12}}{1+\beta r_{12}})\right] = \frac{\md}{\md
            x}\left[\exp(\frac{ar_{12}}{1+\beta r_{12}})\frac{\md}{\md
            x}\exp(-\frac{\alpha\omega}{2}x^2_1) +
            \exp(-\frac{\alpha\omega}{2}x^2_1) \frac{\md}{\md
            x}\exp(\frac{ar_{12}}{1+\beta r_{12}})\right]
        \end{equation}
%             r_{12} = \sqrt{(x_1-x_2)^2 + (y_1-y_2)^2}

        \begin{align*}
            \input{analyticEL2AllCollectNew.tex}
        \end{align*}
