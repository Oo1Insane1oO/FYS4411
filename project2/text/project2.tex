...
\label{ssub:Solution}
    ...
    with solution
        ...
        \end{equation}
    Initially the probability density is centered at $x+D\Delta tF(x)$, that is
    the old position plus the drift term. This can be expressed terms of a
    Dirac-delta function, meaning 
        \begin{equation}
            P(y,x,0)=\delta(y-x-D\Delta tF(x))
            \label{eq:P0}
        \end{equation}
    ...
    trancendental indtegral yields in
        \begin{equation}
    ...
    which gives us the acceptance
        ...
        \end{equation}
    We may rearrange the acceptence ratio giving
        \begin{equation}
            \frac{P(y,x,\Delta t)}{P(x,y,\Delta t)} = \frac{1}{2}
