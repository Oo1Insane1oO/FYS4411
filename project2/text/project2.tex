...
\newcommand{\prodsecrule}[2]{{#2} \nabla^2_k {#1} + {#1} \nabla^2_k {#2}}
    ...
    we have
        \begin{align}
            \prd{x_k}[][2]\left(H_{n_{x_k}}\left(\sqrt{\omega}x_k\right)
            \exp(-\frac{\alpha\omega}{2}x^2_k)\right) &= \omega
            \prd{s}[][2]\left(H_n(s)e(s)\right) \nonumber \\
            &= \omega e(s) \left(\prd{s}[H_n][2] - 2\alpha s\prd{s}[H_n] +
            \alpha(\alpha^2 s^2-1)H_n(s)\right)
            \label{eq:diffxk1}
        \end{align}
    ...
    relation given in \Autoref{eq:hermiteReq} for $H_n(s)$ gives
        \begin{align}
            ...
            \alpha(\alpha^2 s^2-1)(2sH_{n-1}(s)-2(n-1)H_{n-2})\right)e(s)
            \nonumber \\
            &= \omega\left[\frac{n}{s}(4(n+1)-\alpha(\alpha^2 s^2-1))\me^{s^2} +
            2\alpha s(\alpha^2 s^2-2n-3)\right]H_n(s)e(s) \nonumber \\
            \prd{x_k}[][2]\left(H_{n_{x_k}}\left(\sqrt{\omega}x_k\right)
            e(x_k)\right) &= \left[\frac{n_{x_k}\sqrt{\omega}}{x_k}
            \left(4(n_{x_k}+1)-\alpha\left(\alpha^2 \omega
            x^2_k-1\right)\right)\me^{\omega x^2_k} + 2\alpha
            {\omega}^{\frac{3}{2}}x_k(\alpha^2 \omega
            x^2_k-2n_{x_k}-3)\right]H_{n_{x_k}}
            \left(\sqrt{\omega}x_k\right)e(x_k)
            \label{eq:diffs2}
        \end{align}
    We have in the latter step shifted the index $n$ and used
    \Autoref{eq:hermiteExp} to calculate the polynomial ratio. \\
    A similar approach for $y_k$ gives
       \begin{equation}
            \prd{y_k}[][2]\left(H_{n_{y_k}}\left(\sqrt{\omega}y_k\right)
            e\left(y_k\right)\right) = \left[\frac{n_{y_k}\sqrt{\omega}}{y_k}
            \left(4(n_{y_k}+1)-\alpha\left(\alpha^2 \omega
            y^2_k-1\right)\right)\me^{\omega y^2_k} + 2\alpha
            {\omega}^{\frac{3}{2}}y_k(\alpha^2 \omega
            y^2_k-2n_{y_k}-3)\right]H_{n_{y_k}}
            \left(\sqrt{\omega}y_k\right)e(y_k)
            \label{eq:diffyk}
       \end{equation}
    Inserting \Autoref{eq:diffs2,eq:diffyk} into \Autoref{eq:nablakth} with
        \begin{equation}
            \begin{aligned}
                \Theta_{n_{x_k}n_{y_k}}(x_k,y_k,\alpha) \equiv
                \left[\frac{n_{x_k}\sqrt{\omega}}{x_k}
                \left(4(n_{x_k}+1)-\alpha\left(\alpha^2 \omega
                x^2_k-1\right)\right)\me^{\omega x^2_k} + 2\alpha
                {\omega}^{\frac{3}{2}}x_k(\alpha^2 \omega
                x^2_k-2n_{x_k}-3)\right] \\
                + \left[\frac{n_{y_k}\sqrt{\omega}}{y_k}
                \left(4(n_{y_k}+1)-\alpha\left(\alpha^2 \omega
                y^2_k-1\right)\right)\me^{\omega y^2_k} + 2\alpha
                {\omega}^{\frac{3}{2}}y_k(\alpha^2 \omega
                y^2_k-2n_{y_k}-3)\right]
            \end{aligned}
            \label{eq:Ddef}
        \end{equation}
    gives
    ...
    Which gives the final expression for the Laplacian for our wavefunction
        \begin{equation}
            \nabla^2_N\Psi_T =
            \suml{k}{N}\left(D_{n_{x_k}n_{y_k}}(x_k,y_k,\alpha) +
    ...
    and the local energy is thus
        \begin{equation}
            ...
            D_{n_{x_k}n_{y_k}}(x_k,y_k,\alpha) + Q\left(r_k,\beta\right)\right)
            + \sum^N_{i<j} \frac{1}{r_{ij}}
            \label{eq:ELNfin}
        \end{equation}
