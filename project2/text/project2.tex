...
\newcommand{\psinorm}{\int\abs{\psi_T}^2\md^N r}
\newcommand{\psialphder}{\prd{\alpha}[\abs{\psi_T}]}
\newcommand{\psialphsecder}{\prd{\alpha}[\abs{\psi_T}][2]}
\newcommand{\psibetder}{\prd{\beta}[\abs{\psi_T}]}
\newcommand{\psibetsecder}{\prd{\beta}[\abs{\psi_T}][2]}
\newcommand{\gsum}{\sum\limits_{i\neq j} \frac{a}{\left(\beta + \frac{1}{r_{ij}}\right)}}
    ...
    respect to $\alpha$ and $\beta$ is(using \Autoref{eq:hessendef})
        ...
        \end{equation}
    For the first derivative with respect to $\alpha$ we see that we need to
    find the derivative of the product between our PDF and the local energy(see
    \Autoref{ssub:expectation_value_and_local_energy}). This is found by using
    the product rule and the hermiticity of the Hamiltonian, we have
        \begin{align}
            \prd{\alpha}[\ecp{E_L}] &=
            \prd{\alpha}\left(\int\frac{\abs{\psi_T}^2E_L}{\psinorm}\right)
            \nonumber \\
            &= \int\left(\frac{\psinorm\left(2\abs{\psi_T}E_L\psialphder +
            \abs{\psi_T}^2\prd{\alpha}[E_L]\right) -
            2\abs{\psi_T}^2E_L\int\abs{\psi_T}\psialphder\md^N
            r}{\left(\psinorm\right)^2}\right) \md^N r \nonumber \\
            &= 2\ecp{\frac{E_L}{\abs{\psi_T}}\psialphder} -
            2\ecp{E_L}\ecp{\frac{1}{\abs{\psi_T}}\psialphder}
            \label{eq:deralpahecpEl}
        \end{align}
    In the last step the hermiticity of the Hamiltonian was used. \\
    The second derivative is then
        \begin{align}
            \prd{\alpha}[\ecp{E_L}][2] &=
            2\left(\prd{\alpha}\ecp{\frac{E_L}{\abs{\psi_T}}\psialphder} -
            \ecp{E_L}\prd{\alpha}\ecp{\frac{1}{\abs{\psi_T}}\psialphder} -
            \ecp{\frac{1}{\abs{\psi_T}}\psialphder}
            \prd{\alpha}[\ecp{E_L}]\right) \nonumber \\
            &= 2\left(\prd{\alpha}\ecp{\frac{E_L}{\abs{\psi_T}}\psialphder} -
            \ecp{E_L}\prd{\alpha}\ecp{\frac{1}{\abs{\psi_T}}\psialphder} -
            2\ecp{\frac{1}{\abs{\psi_T}}\psialphder}
            \left(\ecp{\frac{E_L}{\abs{\psi_T}}\psialphder} -
            \ecp{E_L}\ecp{\frac{1}{\abs{\psi_T}}\psialphder}\right)\right)
            \label{eq:der2alpahecpElfir}
        \end{align}
    We solve the derivatives individually
        \begin{align}
            \prd{\alpha}\ecp{\frac{E_L}{\abs{\psi_T}}\psialphder} &=
            \prd{\alpha}\left(\int
            \frac{\abs{\psi_T}E_L\psialphder}{\psinorm}\md^N r\right) \nonumber
            \\ 
            &= \int\left(\frac{E_L\left(\psialphder\right)^2 +
            \abs{\psi_T}\prd{\alpha}[E_L]\psialphder +
            \abs{\psi_T}E_L\psialphsecder}{\psinorm} -
            2\frac{\abs{\psi_T}E_L\psialphder}{\psinorm}
            \frac{\int\abs{\psi_T}\psialphder\md^N r}{\psinorm}\right) \md^N r
            \nonumber \\
            &= \ecp{\frac{E_L}{\abs{\psi_T}^2}\left(\psialphder\right)^2} +
            \ecp{\frac{1}{\abs{\psi_T}} \prd{\alpha}[E_L]
            \prd{\alpha}[\abs{\psi_T}]} +
            \ecp{\frac{E_L}{\abs{\psi_T}}\psialphsecder} -
            2\ecp{\frac{E_L}{\abs{\psi_T}}\psialphder}
            \ecp{\frac{1}{\abs{\psi_T}}\psialphder}
            \label{eq:der2alpahecpElfirfir}
        \end{align}
    and
        \begin{align}
            \prd{\alpha}\ecp{\frac{1}{\abs{\psi_T}}\psialphder} &= \prd{\alpha}
            \left(\int\frac{\abs{\psi_T}\psialphder}{\psinorm}\md^N r\right)
            \nonumber \\
            &= \int \left(\frac{\left(\psialphder\right)^2 +
            \abs{\psi_T}\psialphsecder}{\psinorm} -
            2\frac{\abs{\psi_T}\psialphder}{\psinorm} \frac{\int
            \abs{\psi_T}\psialphder\md^N r}{\psinorm}\right)\md^N r \nonumber
            \\
            &= \ecp{\frac{1}{\abs{\psi_T}^2}\left(\psialphder\right)^2} +
            \ecp{\frac{1}{\abs{\psi_T}}\psialphsecder} -
            2\ecp{\frac{1}{\abs{\psi_T}}\psialphder}^2
            \label{eq:der2alpahecpElfirsec}
        \end{align}
    Gathering \Autoref{eq:der2alpahecpElfirfir,eq:der2alpahecpElfirsec} with
    \Autoref{eq:der2alpahecpElfir} we have that the second derivative to the
    expectation value of the local energy with respect to $\alpha$ is
        \begin{align}
            \prd{\alpha}[\ecp{E_L}][2] &=
            2\ecp{\frac{E_L}{\abs{\psi_T}^2}\left(\psialphder\right)^2} +
            2\ecp{\frac{1}{\abs{\psi_T}} \prd{\alpha}[E_L]
            \prd{\alpha}[\abs{\psi_T}]} +
            2\ecp{\frac{E_L}{\abs{\psi_T}}\psialphsecder} -
            6\ecp{\frac{E_L}{\abs{\psi_T}}\psialphder}
            \ecp{\frac{1}{\abs{\psi_T}}\psialphder} \nonumber \\
            &- 2\ecp{E_L} \left(\ecp{\frac{1}{\abs{\psi_T}^2}
            \left(\psialphder\right)^2} -
            \ecp{\frac{1}{\abs{\psi_T}}\psialphsecder} -
            \ecp{\frac{1}{\abs{\psi_T}}\psialphder}^2\right) \\
            \label{eq:ecpElalphasecder}
        \end{align}
    For beta we have the same expression as for $\alpha$(just with $\alpha$
    replaced by $\beta$).
        \begin{align}
            \prd{\beta}[\ecp{E_L}][2] &=
            2\ecp{\frac{E_L}{\abs{\psi_T}^2}\left(\psibetder\right)^2} +
            2\ecp{\frac{1}{\abs{\psi_T}} \prd{\beta}[E_L]
            \prd{\beta}[\abs{\psi_T}]} +
            2\ecp{\frac{E_L}{\abs{\psi_T}}\psibetsecder} -
            6\ecp{\frac{E_L}{\abs{\psi_T}}\psibetder}
            \ecp{\frac{1}{\abs{\psi_T}}\psibetder} \nonumber \\
            &- 2\ecp{E_L} \left(\ecp{\frac{1}{\abs{\psi_T}^2}
            \left(\psibetder\right)^2} -
            \ecp{\frac{1}{\abs{\psi_T}}\psibetsecder} -
            \ecp{\frac{1}{\abs{\psi_T}}\psibetder}^2\right) \\
            \label{eq:ecpElbetasecder}
        \end{align}
    As for the off-diagonal terms we have similar derivations
        \begin{align}
            \frac{\prtl^2\ecp{E_L}}{\prtl\alpha\prtl\beta} &=
            2\left(\prd{\beta}\ecp{\frac{E_L}{\abs{\psi_T}}\psialphder} -
            \ecp{E_L}\prd{\beta}\ecp{\frac{1}{\abs{\psi_T}}\psialphder} -
            \ecp{\frac{1}{\abs{\psi_T}}\psialphder}\prd{\beta}[\ecp{E_L}]
            \right) \nonumber \\
            &= 2\left(\prd{\beta}\ecp{\frac{E_L}{\abs{\psi_T}}\psialphder} -
            \ecp{E_L}\prd{\beta}\ecp{\frac{1}{\abs{\psi_T}}\psialphder} -
            2\ecp{\frac{1}{\abs{\psi_T}}\psialphder}\left(
            \ecp{\frac{E_L}{\abs{\psi_T}}\psibetder} -
            \ecp{E_L}\ecp{\frac{1}{\abs{\psi_T}}\psibetder}\right) \right)
            \label{eq:ecpElbetalpderfir}
        \end{align}
    Solving the derivative terms
        \begin{align}
            \prd{\beta}\ecp{\frac{E_L}{\abs{\psi_T}}\psialphder} &=
            \int\left(\frac{\prd{\alpha}[\abs{\psi_T}]
            \prd{\beta}[\abs{\psi_T}]E_L +
            \abs{\psi_T}\prd{\alpha}[\psi_T]\prd{\beta}[E_L] + \abs{\psi_T}E_L
            \frac{\prtl^2\abs{\psi_T}}{\prtl\alpha\prtl\beta}}{\psinorm} -
            2\frac{\psi_TE_L\prd{\alpha}[\psi_T]}{\psinorm}
            \frac{\int\abs{\psi_T}\prd{\beta}[\psi_T]\md^N
            r}{\psinorm}\right)\md^N r \nonumber \\
            &= \ecp{\frac{E_L}{\abs{\psi_T}^2} \prd{\alpha}[\abs{\psi_T}]
            \prd{\beta}[\abs{\psi_T}]} + \ecp{\frac{1}{\abs{\psi_T}}
            \prd{\alpha}[\abs{\psi_T}] \prd{\beta}[E_L]} +
            \ecp{\frac{E_L}{\abs{\psi_T}}
            \frac{\prtl^2\abs{\psi_T}}{\prtl\alpha\prtl\beta}} -
            2\ecp{\frac{E_L}{\abs{\psi_T}}\prd{\alpha}[\psi_T]}
            \ecp{\frac{1}{\abs{\psi_T}}\prd{\beta}[\abs{\psi_T}]}
            \label{eq:ecpElbetalpderfirfir}
        \end{align}
    and
        \begin{align}
            \prd{\beta}\ecp{\frac{1}{\abs{\psi_T}}\psialphder} &=
            \int\left(\frac{\prd{\alpha}[\abs{\psi_T}]
            \prd{\beta}[\abs{\psi_T}] + \abs{\psi_T}
            \frac{\prtl^2\abs{\psi_T}}{\prtl\alpha\prtl\beta}}{\psinorm} -
            2\frac{\abs{\psi_T} \prd{\alpha}[\abs{\psi_T}]}{\psinorm}
            \frac{\int\abs{\psi_T}
            \prd{\beta}[\abs{\psi_T}]}{\psinorm}\right)\md^N r \nonumber \\
            &= \ecp{\frac{1}{\abs{\psi_T}^2} \prd{\alpha}[\abs{\psi_T}]
            \prd{\beta}[\abs{\psi_T}]} + \ecp{\frac{1}{\abs{\psi_T}}
            \frac{\prtl^2\abs{\psi_T}}{\prtl\alpha\prtl\beta}} -
            2\ecp{\frac{1}{\abs{\psi_T}}\prd{\alpha}[\abs{\psi_T}]}
            \ecp{\frac{1}{\abs{\psi_T}}\prd{\beta}[\abs{\psi_T}]}
            \label{eq:ecpElbetalpderfin1}
        \end{align}
    Thus the off-diagonal elements(which are symmetric due to Schwarz'
    theorem\cite{linalgDavid}) is
        \begin{equation}
            \begin{aligned}
                \frac{\prtl^2\ecp{E_L}}{\prtl\alpha\prtl\beta} &=
                2\ecp{\frac{E_L}{\abs{\psi_T}^2} \prd{\alpha}[\abs{\psi_T}]
                \prd{\beta}[\abs{\psi_T}]} + 2\ecp{\frac{1}{\abs{\psi_T}}
                \prd{\alpha}[\abs{\psi_T}] \prd{\beta}[E_L]} +
                2\ecp{\frac{E_L}{\abs{\psi_T}}
                \frac{\prtl^2\abs{\psi_T}}{\prtl\alpha\prtl\beta}} -
                4\ecp{\frac{E_L}{\abs{\psi_T}} \prd{\alpha}[\abs{\psi_T}]}
                \ecp{\frac{1}{\abs{\psi_T}} \prd{\beta}[\abs{\psi_T}]} \\
                &- 2\ecp{E_L} \ecp{\frac{1}{\abs{\psi_T}^2}
                \prd{\alpha}[\abs{\psi_T}] \prd{\beta}[\abs{\psi_T}]} -
                2\ecp{E_L} \ecp{\frac{1}{\abs{\psi_T}}
                \frac{\prtl^2\abs{\psi_T}}{\prtl\alpha\prtl\beta}} + 4\ecp{E_L}
                \ecp{\frac{1}{\abs{\psi_T}} \prd{\alpha}[\abs{\psi_T}]}
                \ecp{\frac{1}{\abs{\psi_T}} \prd{\beta}[\abs{\psi_T}]} \\
                &- 2\ecp{\frac{1}{\abs{\psi_T}} \prd{\alpha}[\abs{\psi_T}]}
                \ecp{\frac{E_L}{\abs{\psi_T}} \prd{\beta}[\abs{\psi_T}]} +
                2\ecp{E_L} \ecp{\frac{1}{\abs{\psi_T}}
                \prd{\beta}[\abs{\psi_T}]}
            \end{aligned}
            \label{eq:ecpElbetalpderfin2}
        \end{equation}
    For the derivative of the wavefunction We find the elements individually by
    using Jacobi's formula as described in
    ...
    \Autoref{eq:psideralpha,eq:psiderbeta} giving
        \begin{align}
            ...
            j}\frac{1}{\left(\beta + \frac{1}{r_{ij}}\right)}
            \label{eq:psideralpbetsec}
        \end{align}
    The final expression for the elements in the Hessen matrix is thus
        \begin{equation}
            \begin{aligned}
                \prd{\alpha}[\ecp{E_L}][2] &= \omega^2\ecp{E_LR^2} -
                \omega\ecp{\prd{\alpha}[E_L]R} +
                \frac{\omega^2}{2}\left(\ecp{E_LR^2} + \ecp{E_L}\ecp{R}^2 -
                3\omega\ecp{E_LR}\ecp{R}\right) \\
                \prd{\beta}[\ecp{E_L}][2] &= 4\ecp{E_Lh^2} -
                2\ecp{\prd{\beta}[E_L]h} + 2\left(\ecp{E_Lh^2} +
                2\ecp{E_L}\ecp{h}^2\right) -6\ecp{E_Lh}\ecp{h} +
                4\left(\ecp{l}\ecp{E_L} + \ecp{E_Ll}\right) \\
                \frac{\prtl^2\ecp{E_L}}{\prtl\alpha\prtl\beta} &=
                2\omega\left[\ecp{E_LRh} - \ecp{R}\ecp{E_Lh}\right] -
                2\ecp{h}\left[\ecp{E_L}\left(2 - \omega\ecp{R}\right) +
                \omega\ecp{E_LR}\right]
            \end{aligned}
    ...
    where 
        \begin{equation}
            \begin{aligned}
                ...
                h &\equiv \gsum \\
                l &\equiv \sum\limits_{i\neq j} \frac{a}{\left(\beta +
                \frac{1}{r_{ij}}\right)^3}
            \end{aligned}
            \label{eq:rhdef}
        \end{equation}
