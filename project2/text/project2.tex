...
{#2}}
    ...
    in \Autoref{eq:hermiteReq} for $H_n(s)$ gives
        \begin{align}
            ...
            \alpha\omega\left(\prd{x}[H_n][2] - 2\alpha s\prd{x}[H_n] +
            \alpha\left(\alpha s^2 - 1\right)H_n(s)\right)e(s) \nonumber \\
            &= \alpha\omega\left(8n(n-1)H_{n-2}(s) - 4n\alpha sH_{n-1}(s) +
            \alpha(\alpha s^2-1)(2sH_{n-1}(s)-2(n-1)H_{n-2})\right)e(s)
            \nonumber \\
            &= \alpha\omega \left[2(n-1)\left(4n - \alpha\left(\alpha s^2 -
            ...
            \nonumber \\
            &= \alpha\omega \left[2n(n-1)(2-\alpha)\frac{H_{n-2}}{H_n} +
    ...
    the first derivatives to be
        \begin{align}
            ...
            \sqrt{\alpha\omega}\left(\prd{s}[H_n] - \alpha sH_n\right)e(s)
            \nonumber \\
            &= \sqrt{\alpha\omega}\left(2nH_{n-1} - \alpha sH_n\right)e(s)
            \nonumber \\
            &= \sqrt{\alpha\omega}\left(2n\frac{H_{n-1}}{H_n} - \alpha
            s\right)H_n(s)e(s)
            \label{eq:sfirstder}
        \end{align}
    The ratios for the one-body part are thus
        \begin{equation}
            \begin{aligned}
                ...
                \\
                &= \suml{k}{N}
                \left(2\sqrt{\alpha\omega}
                \left(n_{x_j}\frac{H_{n_{x_j}-1}\left(\sqrt{\alpha\omega}x_k\right)}
                {H_{n_{x_j}}\left(\sqrt{\alpha\omega}x_k\right)}\right) -
                \alpha^2\omega x_k, 2\sqrt{\alpha\omega}
                \left(n_{y_j}\frac{H_{n_{y_j}-1}\left(\sqrt{\alpha\omega}y_k\right)}
                {H_{n_{y_j}}\left(\sqrt{\alpha\omega}y_k\right)}\right) -
                \alpha^2\omega y_k\right)
            \end{aligned}
    ...
    and the Laplacian
        \begin{equation}
            \begin{aligned}
                ...
                \alpha\left(\alpha^2 \omega r_k^2 - 2\left(n_{x_k} +
