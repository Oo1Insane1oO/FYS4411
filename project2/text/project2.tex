...
\newcommand{\gsum}{\sum\limits_{i\neq j} \frac{a}{\left(\beta + \frac{1}{r_{ij}}\right)}}
    ...
    the product rule and the hermiticity of the Hamiltonian, we have
        \begin{align}
            \prd{\alpha}[\ecp{E_L}] &=
            \prd{\alpha}\left(\int\frac{\psi_T^2E_L}{\psinorm}\right)
            \nonumber \\
            &= \int\left(\frac{\psinorm\left(2\psi_TE_L\psialphder +
            \psi_T^2\prd{\alpha}[E_L]\right) -
            2\psi_T^2E_L\int\psi_T\psialphder\md\tau}
            {\left(\psinorm\right)^2}\right) \md\tau \nonumber \\
            &= 2\ecp{\frac{E_L}{\psi_T}\psialphder} -
            2\ecp{E_L}\ecp{\frac{1}{\psi_T}\psialphder}
    ...
    The second derivative is then
        \begin{align}
            \prd{\alpha}[\ecp{E_L}][2] &=
            2\left(\prd{\alpha}\ecp{\frac{E_L}{\psi_T}\psialphder} -
            \ecp{E_L}\prd{\alpha}\ecp{\frac{1}{\psi_T}\psialphder} -
            \ecp{\frac{1}{\psi_T}\psialphder}
            \prd{\alpha}[\ecp{E_L}]\right) \nonumber \\
            &= 2\left(\prd{\alpha}\ecp{\frac{E_L}{\psi_T}\psialphder} -
            \ecp{E_L}\prd{\alpha}\ecp{\frac{1}{\psi_T}\psialphder} -
            2\ecp{\frac{1}{\psi_T}\psialphder}
            \left(\ecp{\frac{E_L}{\psi_T}\psialphder} -
            \ecp{E_L}\ecp{\frac{1}{\psi_T}\psialphder}\right)\right)
            \label{eq:der2alpahecpElfir}
        \end{align}
    We solve the derivatives individually
        \begin{align}
            \prd{\alpha}\ecp{\frac{E_L}{\psi_T}\psialphder} &=
            \prd{\alpha}\left(\int
            \frac{\psi_TE_L\psialphder}{\psinorm}\md\tau\right) \nonumber
            \\ 
            &= \int\left(\frac{E_L\left(\psialphder\right)^2 +
            \psi_T\prd{\alpha}[E_L]\psialphder +
            \psi_TE_L\psialphsecder}{\psinorm} -
            2\frac{\psi_TE_L\psialphder}{\psinorm}
            \frac{\int\psi_T\psialphder\md\tau}{\psinorm}\right) \md\tau
            \nonumber \\
            &= \ecp{\frac{E_L}{\psi_T^2}\left(\psialphder\right)^2} +
            \ecp{\frac{1}{\psi_T} \prd{\alpha}[E_L]
            \prd{\alpha}[\psi_T]} +
            \ecp{\frac{E_L}{\psi_T}\psialphsecder} -
            2\ecp{\frac{E_L}{\psi_T}\psialphder}
            \ecp{\frac{1}{\psi_T}\psialphder}
    ...
    and
        \begin{align}
            \prd{\alpha}\ecp{\frac{1}{\psi_T}\psialphder} &= \prd{\alpha}
            \left(\int\frac{\psi_T\psialphder}{\psinorm}\md\tau\right)
            \nonumber \\
            &= \int \left(\frac{\left(\psialphder\right)^2 +
            \psi_T\psialphsecder}{\psinorm} -
            2\frac{\psi_T\psialphder}{\psinorm} \frac{\int
            \psi_T\psialphder\md\tau}{\psinorm}\right)\md\tau \nonumber
            \\
            &= \ecp{\frac{1}{\psi_T^2}\left(\psialphder\right)^2} +
            \ecp{\frac{1}{\psi_T}\psialphsecder} -
            2\ecp{\frac{1}{\psi_T}\psialphder}^2
    ...
    expectation value of the local energy with respect to $\alpha$ is
        \begin{align}
            \prd{\alpha}[\ecp{E_L}][2] &=
            2\left(\ecp{\frac{E_L}{\psi_T^2}\left(\psialphder\right)^2} +
            \ecp{\frac{1}{\psi_T} \prd{\alpha}[E_L] \prd{\alpha}[\psi_T]} +
            \ecp{\frac{E_L}{\psi_T}\psialphsecder} -
            4\ecp{\frac{E_L}{\psi_T}\psialphder}
            \ecp{\frac{1}{\psi_T}\psialphder}\right) \nonumber \\
            &- 2\ecp{E_L} \left(\ecp{\frac{1}{\psi_T^2}
            \left(\psialphder\right)^2} - \ecp{\frac{1}{\psi_T}\psialphsecder}
            - 2\ecp{\frac{1}{\psi_T}\psialphder}^2\right) \\
    ...
    replaced by $\beta$).
        \begin{align}
            \prd{\beta}[\ecp{E_L}][2] &=
            2\left(\ecp{\frac{E_L}{\psi_T^2}\left(\psibetder\right)^2} +
            \ecp{\frac{1}{\psi_T} \prd{\beta}[E_L] \prd{\beta}[\psi_T]} +
            \ecp{\frac{E_L}{\psi_T}\psibetsecder} -
            4\ecp{\frac{E_L}{\psi_T}\psibetder}
            \ecp{\frac{1}{\psi_T}\psibetder}\right) \nonumber \\
            &- 2\ecp{E_L} \left(\ecp{\frac{1}{\psi_T^2}
            \left(\psibetder\right)^2} - \ecp{\frac{1}{\psi_T}\psibetsecder}
            - 2\ecp{\frac{1}{\psi_T}\psibetder}^2\right) \\
            \label{eq:ecpElbetasecder}
        \end{align}
    As for the off-diagonal terms we have similar derivations
        \begin{align}
            \frac{\prtl^2\ecp{E_L}}{\prtl\alpha\prtl\beta} &=
            2\left(\prd{\beta}\ecp{\frac{E_L}{\psi_T}\psialphder} -
            \ecp{E_L}\prd{\beta}\ecp{\frac{1}{\psi_T}\psialphder} -
            \ecp{\frac{1}{\psi_T}\psialphder}\prd{\beta}[\ecp{E_L}]
            \right) \nonumber \\
            &= 2\left(\prd{\beta}\ecp{\frac{E_L}{\psi_T}\psialphder} -
            \ecp{E_L}\prd{\beta}\ecp{\frac{1}{\psi_T}\psialphder} -
            2\ecp{\frac{1}{\psi_T}\psialphder}\left(
            \ecp{\frac{E_L}{\psi_T}\psibetder} -
            \ecp{E_L}\ecp{\frac{1}{\psi_T}\psibetder}\right) \right)
            \label{eq:ecpElbetalpderfir}
        \end{align}
    Solving the derivative terms
        \begin{align}
            \prd{\beta}\ecp{\frac{E_L}{\psi_T}\psialphder} &=
            \int\left(\frac{\prd{\alpha}[\psi_T]
            \prd{\beta}[\psi_T]E_L +
            \psi_T\prd{\alpha}[\psi_T]\prd{\beta}[E_L] + \psi_TE_L
            \frac{\prtl^2\psi_T}{\prtl\alpha\prtl\beta}}{\psinorm} -
            2\frac{\psi_TE_L\prd{\alpha}[\psi_T]}{\psinorm}
            \frac{\int\psi_T\prd{\beta}[\psi_T]\md^N
            r}{\psinorm}\right)\md\tau \nonumber \\
            &= \ecp{\frac{E_L}{\psi_T^2} \prd{\alpha}[\psi_T]
            \prd{\beta}[\psi_T]} + \ecp{\frac{1}{\psi_T}
            \prd{\alpha}[\psi_T] \prd{\beta}[E_L]} +
            \ecp{\frac{E_L}{\psi_T}
            \frac{\prtl^2\psi_T}{\prtl\alpha\prtl\beta}} -
            2\ecp{\frac{E_L}{\psi_T}\prd{\alpha}[\psi_T]}
            \ecp{\frac{1}{\psi_T}\prd{\beta}[\psi_T]}
            \label{eq:ecpElbetalpderfirfir}
        \end{align}
    and
        \begin{align}
            \prd{\beta}\ecp{\frac{1}{\psi_T}\psialphder} &=
            \int\left(\frac{\prd{\alpha}[\psi_T]
            \prd{\beta}[\psi_T] + \psi_T
            \frac{\prtl^2\psi_T}{\prtl\alpha\prtl\beta}}{\psinorm} -
            2\frac{\psi_T \prd{\alpha}[\psi_T]}{\psinorm}
            \frac{\int\psi_T
            \prd{\beta}[\psi_T]}{\psinorm}\right)\md\tau \nonumber \\
            &= \ecp{\frac{1}{\psi_T^2} \prd{\alpha}[\psi_T]
            \prd{\beta}[\psi_T]} + \ecp{\frac{1}{\psi_T}
            \frac{\prtl^2\psi_T}{\prtl\alpha\prtl\beta}} -
            2\ecp{\frac{1}{\psi_T}\prd{\alpha}[\psi_T]}
            \ecp{\frac{1}{\psi_T}\prd{\beta}[\psi_T]}
    ...
    theorem\cite{linalgDavid}) is
        \begin{equation}
            \begin{aligned}
                \frac{\prtl^2\ecp{E_L}}{\prtl\alpha\prtl\beta} &=
                2\ecp{\frac{E_L}{\psi_T^2} \prd{\alpha}[\psi_T]
                \prd{\beta}[\psi_T]} + 2\ecp{\frac{1}{\psi_T}
                \prd{\alpha}[\psi_T] \prd{\beta}[E_L]} +
                2\ecp{\frac{E_L}{\psi_T}
                \frac{\prtl^2\psi_T}{\prtl\alpha\prtl\beta}} -
                4\ecp{\frac{E_L}{\psi_T} \prd{\alpha}[\psi_T]}
                \ecp{\frac{1}{\psi_T} \prd{\beta}[\psi_T]} \\
                &- 2\ecp{E_L} \ecp{\frac{1}{\psi_T^2}
                \prd{\alpha}[\psi_T] \prd{\beta}[\psi_T]} -
                2\ecp{E_L} \ecp{\frac{1}{\psi_T}
                \frac{\prtl^2\psi_T}{\prtl\alpha\prtl\beta}} + 4\ecp{E_L}
                \ecp{\frac{1}{\psi_T} \prd{\alpha}[\psi_T]}
                \ecp{\frac{1}{\psi_T} \prd{\beta}[\psi_T]} \\
                &- 2\ecp{\frac{1}{\psi_T} \prd{\alpha}[\psi_T]}
                \ecp{\frac{E_L}{\psi_T} \prd{\beta}[\psi_T]} +
                2\ecp{E_L} \ecp{\frac{1}{\psi_T}
                \prd{\beta}[\psi_T]} \ecp{\frac{1}{\psi_T}
                \prd{\alpha}[\psi_T]}
            \end{aligned}
