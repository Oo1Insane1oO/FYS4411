...
{#2}}
    ...
    We define a new function
        \begin{equation}
            Q\left(r_k,\beta\right) \equiv \sum_{j>k} \frac{2a}{(1+\beta
    ...
    in \Autoref{eq:PhiLapexp} with \Autoref{eq:diffs2} gives
        \begin{align}
            ...
            \nonumber \\
            &= \Phi_{kj} \sqrt{\omega}\left(2n_{x_k}\frac{H_{n_{x_k}-1}}{H_{n_{x_k}}} - \alpha
            \sqrt{\omega}x_k\right)
            \label{eq:detder1}
        \end{align}
    For $g(\vec{r},\beta)$ we have with \Autoref{eq:lapg,eq:derxkg}
        \begin{align}
            ...
            \gJ\right) \nonumber \\
            &= g(\vec{r},\beta) \sum_{j>k}\prd{x_k}\left(\frac{a}{\beta +
            \frac{1}{r_{kj}}}\right) \nonumber \\
            &= g(\vec{r},\beta) \sum_{j>k} \frac{a\left(x_k-x_j\right)}
    ...
    Defining
        \begin{equation}
            \Upsilon_{n_{x_k}n_{y_k}}(x_k,y_k,\alpha) \equiv \sum_{j>k}
            \frac{4a\sqrt{\omega}}{r_{kj}\left(1+\beta r_{kj}\right)^2}
            \left(2n_{x_k}\frac{H_{n_{x_k}-1}}{H_{n_{x_k}}}\left(x_k-x_j\right)
            + 2n_{y_k}\frac{H_{n_{y_k}-1}}{H_{n_{y_k}}}\left(y_k-y_j\right) -
            \alpha\sqrt{\omega}
            \left(x_k\left(x_k-x_j\right)+y_k\left(y_k-y_j\right)\right)\right)
            \label{eq:upsilondef}
        \end{equation}
