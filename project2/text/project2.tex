...
\label{ssub:Solution}
    ...
    coordinate $r$ according to \Autoref{eq:FourierTransform} giving the equation
        \begin{equation}
            \prd{t}[\tilde{P}(s,t)] = -\frac{D}{\sqrt{2\pi}}s^2\tilde{P}(s,t)
            \label{eq:PdiffFourier}
        \end{equation}
    with solution
        \begin{equation}
            \tilde{P}(s,\Delta t) = \tilde{P}(s,0)
            \me^{-\frac{D}{\sqrt{2\pi}}s^2\Delta t}
            \label{eq:PdiffFourierSol}
        \end{equation}
    Initially the probability density is centered at $x$, that is the old
    position. This can be expressed terms of a Dirac-delta function, meaning
    $P(x,0)=\delta(y-x)$ (where $y$ is given in \Autoref{eq:newPos}). Making an
    inverse transformation as described in \Autoref{eq:invFourier} yields in
        \begin{equation}
            P(x,\Delta t) = \frac{1}{(4\pi D\Delta
            t)^{\frac{3N}{2}}}\exp(-\frac{(y-x)^2}{4D\Delta t})
        \end{equation}
...
\label{sub:inverse_of_matrix}
    ...
    of a matrix in that case. The formula is
        \begin{equation}
            ...
            \label{eq:InvUpdate}
        \end{equation}
    as described by Sherman and Morris >>REF HERE<<.
