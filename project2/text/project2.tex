...
\label{sub:fourier_transformation}
    ...
    transform to variable $x$ is defined to be
        \begin{equation}
            \tilde{f}(k) = \infint f(x)\me^{-2\pi ixs} \md x
            \label{eq:FourierTransform}
        \end{equation}
    with $x,s\in\mathbb{R}$. \\
    The inverse transformation is given as
        \begin{equation}
            f(x) = \infint \tilde{f}(k)\me^{2\pi isx} \md s
            \label{eq:invFourier}
        \end{equation}
...
\label{ssub:Solution}
    ...
    following diffusion equation
        \begin{equation}
            ...
            \label{eq:Pdiff}
        \end{equation}
    This equation is solved by using a Fourier transform in the spatial coordinate $r$ according to \Autoref{eq:FourierTransformt a}
