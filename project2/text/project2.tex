...
{#2}}
    ...
    Starting by solving $\nabla^2_k\det(\Phi(\vec{r},\alpha))$ with Jacobi's
    formula given in \Autoref{sub:derivative_of_determinant}. We see that we
    need to derivative to the individual elements of the Slater matrix $\Phi$.
    These are with the substitution substituting
    $s=\sqrt{\omega}x_k=\sqrt{\omega}y_k$ and labeling $n_x,n_y\rarr n$,
        \begin{align}
            ...
            &= \sqrt{\omega} H_{n_{y_j}}\left(\sqrt{\omega}y_k\right)
            \me^{-\frac{\alpha\omega}{2}r^2_k}
            H_{n_{x_j}}\left(\sqrt{\omega}x_k\right)
            \left(\frac{n_{x_j}}{\sqrt{\omega}x_k}
            \left(1+2\left(n_{x_j}-1\right)
            \frac{H_{n_{x_j}-2}}{H_{n_{x_j}}}\right) -
            \alpha\sqrt{\omega}x_k\right) \nonumber \\
            &= \Phi_{ij}\left(\frac{n_{x_j}}{x_k}
            \left(1+2\left(n_{x_j}-1\right)
            \frac{H_{n_{x_j}-2}}{H_{n_{x_j}}}\right) - \alpha\omega x_k\right)
            \nonumber \\
