...
\newcommand{\adj}[1]{\text{adj}\left(#1\right)}
    ...
    respect to $\alpha$ and $\beta$ is(using \Autoref{eq:hessendef})
        ...
        \end{equation}
    For the second derivative with respect to $\alpha$ we see that we need to
    ...
    the product rule and the hermiticity of $\psi_T$, we have
        \begin{align}
            \prd{\alpha}[\ecp{E_L}][2] &=
            \prd{\alpha}\left(\int\frac{1}{\int\abs{\psi_T}^2\md^N
            r}\prd{\alpha}\left(\abs{\psi_T}^2E_L\right)\md^N r\right)
            \nonumber \\
            &= \prd{\alpha}\left(\int \frac{1}{\int\abs{\psi_T}^2\md^N r}
            \left(2\psi_TE_L\prd{\alpha}[\psi_T] +
            \abs{\psi_T}^2\prd{\alpha}[E_L]\right)\md^N r \right) \nonumber \\
            &= \int \frac{1}{\int\abs{\psi_T}^2\md^N r}
            \left(2E_L\left(\left(\prd{\alpha}[\psi_T]\right)^2 +
            \psi_T\prd{\alpha}[\psi_T][2]\right) +
            2\psi_T\prd{\alpha}[\psi_T]\prd{\alpha}[E_L]+
            2\psi_T\prd{\alpha}[\psi_T]\prd{\alpha}[E_L] +
            \abs{\psi_T}^2\prd{\alpha}[E_L][2]\right) \md^N r \nonumber \\
            &= 2\left(\ecp{\frac{1}{\abs{\psi_T}^2}\left(\prd{\alpha}[\abs{\psi_T}]\right)^2E_L} + \ecp{\frac{E_L}{\abs{\psi_T}}\prd{\alpha}[\abs{\psi_T}][2]} + 2\ecp{\frac{1}{\abs{\psi_T}}\prd{\alpha}[\abs{\psi_T}]\prd{\alpha}[E_L]} + \ecp{\prd{\alpha}[E_L][2]}\right)
