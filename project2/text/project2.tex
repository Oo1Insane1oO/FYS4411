...
{#2}}
    ...
    the single particle wave functions.  The single particle wave function is
        \begin{equation}
    ...
    the first derivatives to be
        \begin{align}
            \sqrt{\omega}\prd{s}\left(H_n(s)e(s)\right) &=
            \sqrt{\omega}\left(\prd{s}[H_n] - \alpha sH_n\right)e(s) \nonumber
            \\
            &= \sqrt{\omega}\left(2nH_{n-1} - \alpha sH_n\right)e(s) \nonumber
            \\
            &= \sqrt{\omega}\left(2n\frac{H_{n-1}}{H_n} - \alpha
            s\right)H_n(s)e(s)
            \label{eq:sfirstder}
        \end{align}
    The ratios for the one-body part are thus
        \begin{equation}
            \begin{aligned}
                \frac{\nabla\det(\Phi)}{\det(\Phi)} &= \suml{k}{N} \nabla_k \phi_j\left(\vec{r}_k,\alpha\right)\phi^{-1}_{jk}\left(\vec{r},\alpha\right) \\
                &= \suml{k}{N}
                \left[2\sqrt{\omega}\left(n_{x_j}\frac{H_{n_{x_j}-1}\left(\sqrt{\omega}x_k\right)}
                {H_{n_{x_j}}\left(\sqrt{\omega}x_k\right)} +
                n_{y_j}\frac{H_{n_{y_j}-1}\left(\sqrt{\omega}y_k\right)}
                {H_{n_{y_j}}\left(\sqrt{\omega}y_k\right)}\right) -
                \alpha\omega\left(x_k + y_k\right)\right]
            \end{aligned}
    ...
    and the Laplacian
        \begin{equation}
            \begin{aligned}
                ...
                \phi_j\left(\vec{r}_k,\alpha\right)\phi^{-1}_{jk}\left(\vec{r},\alpha\right)
                \\
                &= \suml{k}{N} \left[2\omega
                \left(2-\alpha\right)\left[n_{x_k}\left(n_{x_k}-1\right)
