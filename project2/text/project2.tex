...
\newcommand{\gsum}{\sum\limits_{i\neq j} \frac{a}{\left(\beta + \frac{1}{r_{ij}}\right)}}
    ...
    Solving the derivative terms
        \begin{align}
            ...
            \prd{\beta}[\abs{\psi_T}]} + \ecp{\frac{E_L}{\abs{\psi_T}}
    ...
    theorem\cite{linalgDavid}) is
        \begin{equation}
            \begin{aligned}
                ...
                \prd{\beta}[\abs{\psi_T}]} + 2\ecp{\frac{E_L}{\abs{\psi_T}}
                \frac{\prtl^2\abs{\psi_T}}{\prtl\alpha\prtl\beta}} - 2\ecp{E_L}
                \left(\ecp{\frac{1}{\abs{\psi_T}^2} \prd{\alpha}[\abs{\psi_T}]
                \prd{\beta}[\abs{\psi_T}]} - \ecp{\frac{1}{\abs{\psi_T}}
                \frac{\prtl^2\abs{\psi_T}}{\prtl\alpha\prtl\beta}}\right) \\
                &+ 4\ecp{\frac{1}{\abs{\psi_T}}\prd{\beta}[\abs{\psi_T}]}
                \left(\ecp{E_L}\left(1+\ecp{\frac{1}{\abs{\psi_T}}
                \prd{\alpha}[\abs{\psi_T}]}\right) -
                \ecp{\frac{E_L}{\abs{\psi_T}}
                \prd{\alpha}[\abs{\psi_T}]}\right) -
                4\ecp{\frac{1}{\abs{\psi_T}} \prd{\alpha}[\abs{\psi_T}]}
    ...
    The final expression for the elements in the Hessen matrix is thus
        \begin{equation}
            \begin{aligned}
                \prd{\alpha}[\ecp{E_L}][2] &=
                \frac{\omega^2}{2}\left(1-\ecp{E_L}\right) \ecp{E_LR^2} -
                4\omega\ecp{E_LR} \ecp{R} - \frac{\omega^2}{2}\ecp{E_L}
                \ecp{R^2} + 2\omega^2\ecp{E_L}\ecp{R}^2 \\
                \prd{\beta}[\ecp{E_L}][2] &= 2\left(1-\ecp{E_L}\right)
                \ecp{E_Lh^2} - 8\ecp{E_Lh^2}\ecp{h^2} - 2\ecp{h^2} +
                2\ecp{\sum\limits_{i\neq j}\frac{a}{\left(\beta +
                \frac{1}{r_{ij}}\right)^3}} + 8\ecp{E_L}\ecp{h}^2 \\
                \frac{\prtl^2\ecp{E_L}}{\prtl\alpha\prtl\beta} &=
