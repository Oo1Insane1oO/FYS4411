...
\label{sub:metropolis_algorithm}
    ...
    functions satisfy the following condition
        ...
        \end{equation}
    We now need to choose an acceptance which fulfills \Autoref{eq:detBalw} and
    a common choice is the Metropolis condition
        \begin{equation}
            \ufij{A}{j}{i} = \min\left(1,
            \frac{w_i}{w_j}\frac{\ufij{T}{i}{j}}{\ufij{T}{j}{i}}\right)
...
\label{ssub:EXPECTATION VALUE AND LOCAL ENERGY}
    ...
    algorithm >>INSERT REF<< the expectation value,
    ...
    \Autoref{eq:HupperBoundExpand} can be rewritten as
        \begin{equation}
            ...
            \label{eq:ecpRew}
        \end{equation}
    which is of the form given in >>INSERT REF HERE<<.
