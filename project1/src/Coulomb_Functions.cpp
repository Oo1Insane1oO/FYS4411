        ...
        for (int j2 = 0; j2 <= nj; ++j2) {
            for (int j3 = 0; j3 <= nl; ++j3) {
                for (int j4 = 0; j4 <= nk; ++j4) {
                    ...
                    dir += (-2*((j1 + j2 + j3 + j4)%2) + 1) *
                        std::exp(LogRatio1 + LogProd2 + LogRatio2) * temp;
                }
            }
        }
    }
...
double logfac(int &n) {
    if (n < 0) {
        std::cerr << n << " : LogFactorial intput should be >= -1" <<
            std::endl; exit(1);
    }
    double fac = 0.0;
    for (int a = 2; a < n+1; a++) { 
...
double product1(int &n1, int &m1, int &n2, int &m2, int &n3, int &m3, int &n4,
        int &m4) {
...
double logproduct2(int &n1, int &m1, int &n2, int &m2, int &n3, int &m3, int
        &n4, int &m4, int &j1, int &j2, int &j3, int &j4) {
    int arg1 = n1 + std::abs(m1);
    int arg2 = n2 + std::abs(m2);
    int arg3 = n3 + std::abs(m3);
    int arg4 = n4 + std::abs(m4);
...
double logproduct3(int &l1, int &l2, int &l3, int &l4, int &g1, int &g2, int
        &g3, int &g4) {
    int garg1 = g1 - l1;
    int garg2 = g2 - l2;
    int garg3 = g3 - l3;
